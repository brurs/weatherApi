...
namespace Domain.Handler
{
    public sealed class GetWeatherListHandler : IRequestHandler<GetWeatherListQueryRequest, IEnumerable<GetWeatherListQueryResult>>
    {
        ...
        {
            return Task.FromResult(_data.Weather.Where(x => request.Cidade.Contains(x.Cidade) && x.Data >= request.Inicio && x.Data <= request.Fim).Select(x => new GetWeatherListQueryResult(x)));
        }
    }
}
